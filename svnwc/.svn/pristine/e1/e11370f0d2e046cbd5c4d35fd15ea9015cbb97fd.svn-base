...
void TCDataCollectionJTT808Handler::DoCommand_LocInfo(TCCustomUniSocket  &cusSocket){
		...
		//unsigned char cTag = (unsigned char)sAdditonalLocInfo_Str[1];
		//printf("cTag:%d\n", cTag);
		//printf("cTag:%d\n", 0xf1);
		//
		//if(cTag != 0xf1){
		//	//:不是透传包的ibeacon,丢弃处理			
		//	sLog = "Additional Data Lac-Cell Error Msg";
		//	glgMls->AddDataCollectionRunLog(sLog);
		//	
		//	sLog="Additinal Lac-Cell Info_Str: Length=["+IntToStr(Length(sAdditonalLocInfo_Str))+"]\n";
		//	for(int i=1;i<= Length(sAdditonalLocInfo_Str);i++)
		//	{
		//		char sBuff[8];
		//		memset(sBuff, 0, sizeof(sBuff));
		//		sprintf(sBuff, "02x", sAdditonalLocInfo_Str[i]);
		//		TCString sTempValue=TCString(sBuff);
		//		sLog+="["+Right(sTempValue, 2)+"]:";
		//	}
		//	sLog+="\n";
		...
		sLog="DoCommand_LocInfo: sMsisdn=["+m_sMsisdn+"], nLAC-CI=["+IntToStr(nLAC)+"-"+IntToStr(nCellID)+"],nRscp=["+IntToStr(nRscp)+"],nRxLev=["+IntToStr(nRxLev)+"], GPS=["+FloatToStr(sPosNode.dLongitude,6)+","+FloatToStr(sPosNode.dLatitude,6)+"], dHigh=["+FloatToStr(dHigh,3)+"], dPower=["+FloatToStr(dPower,3)+"]\n";
		glgMls->AddDataCollectionRunLog(sLog);
				
		RecordTrackerLocInfo(m_sMsisdn, sPosNode, sLBSNode, nRscp, nRxLev, fSpeed, dHigh);
...
bool TCDataCollectionJTT808Handler::LoadImsiMore()
{
	TCString sSQL = "SELECT phone_no, imsi, imei from wudongde_imsi_more ";
	...
	while (!otlParamQry.eof()) {
		...
		char tem[32] = { 0 };
		otlParamQry >> phone_no;
		otlParamQry >> imsi;
		otlParamQry >> imei;

		ImsiMore imsi_more;
		imsi_more.imei_ = TCString(imei);
		imsi_more.imsi_ = TCString(imsi);
		TCString tc_no(phone_no);

		mapImsiMore[tc_no] = imsi_more;
	}
}
...
bool TCDataCollectionJTT808Handler::RecordTrackerLocInfo(TCString phone_no, str_Space_Pos_Unit sPosNode\
	, str_Lac_Cell_Node sLBSNode\
	, int nRscp, int nRxLev, double nspeed, double high){
	try{  
		RecordInfo record_info;
		record_info.clear();
		////: 插入数据到结果表中
		//TCString sSQL="INSERT INTO MB_POSTITON_LIST(day,UNIQUEID,X,Y,";
		//sSQL+="LAC,CELLID,CELLX, CELLY, RSCP, RXLEV";
		//sSQL+=")";
		//sSQL+="VALUES(";
		//sSQL+="sysdate, :F1IMEI<char[19]>,:F2LONGTITUDE<char[33]>, :F3LATITUDE<char[33]>, ";
		////: LBS 定位的时候，需要增加小区信息和场强
		//sSQL+=" :F4LAC<int>, :F5CELLID<int>, :F6CELLX<char[33]>, :F7CELLY<char[33]>, :F8RSCP<int>, :F9RXLEV<int>";
		//sSQL+=")";
  //  
  //  otl_stream otlParamQry(1, // buffer size
  //                    (char*)sSQL,
  //                     m_dbConnect);// connect object
    ...
    map<str_Lac_Cell_Node, str_Space_Pos_Unit>::iterator ITR_AdjLacCell_PosUnit;
		...
		map<TCString, ImsiMore>::iterator imsi_iter;
		imsi_iter = mapImsiMore.find(record_info.phone_no);
		if (imsi_iter != mapImsiMore.end())
		{
			ImsiMore imis_more_tmp = imsi_iter->second;
			record_info.imsi = imis_more_tmp.imsi_;
			record_info.unique_id_ = imis_more_tmp.imei_;
		}
