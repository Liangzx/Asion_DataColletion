...
private:
  ...
  void RecvRequest(TCCustomUniSocket  &cusSocket);
    ...
    void Init();
		...
		bool RecordTrackerLocInfo(TCString phone_no, str_Space_Pos_Unit sPosNode, str_Lac_Cell_Node sLBSNode, int nRscp, int nRxLev, double nspeed=0.0, double high = 0.0);
...
private:
		...
		//: 终端心跳
		void DoCommand_HeartBreak(TCCustomUniSocket  &cusSocket);
...
public:
    ...
    ~TCDataCollectionJTT808Handler();
   	bool Main_Handler(TCCustomUniSocket  &cusSocket);
		//TODO:
		std::map<TCString, ImsiMore> mapImsiMore;
};
