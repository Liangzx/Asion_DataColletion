...
void TCDataCollectionConfig::LoadIniFile(){
    ...
    sReadStr = ProfileAppString(Application.GetAppName(), "GENERAL","DB_CONSTR", "");
    sReadStr=AllTrim(sReadStr);
    sReadStr=TrimCRLF(sReadStr);	//: 去掉回车换行
    m_sDBConStr=sReadStr;
    ASSERT( m_sDBConStr != "" );

		sReadStr = ProfileAppString(Application.GetAppName(), "GENERAL","VENDOR", "");
    sReadStr=AllTrim(sReadStr);
    sReadStr=TrimCRLF(sReadStr);	//: 去掉回车换行
    m_sTracker_Vendor=sReadStr;
    ASSERT( m_sTracker_Vendor != "" );

	sReadStr = ProfileAppString(Application.GetAppName(), "GENERAL", "UPDATE", "FALSE");
	jtt_update_flag = (sReadStr == "TRUE") ? true : false;
	sReadStr = ProfileAppString(Application.GetAppName(), "GENERAL", "UPDATE_PARAM_STR", "");
	sReadStr = AllTrim(sReadStr);
	jtt_update_param_str = sReadStr;
    ...
    sReadStr=TrimCRLF(sReadStr);	//: 去掉回车换行
     if (sReadStr != "") 
    	m_nPort = StrToInt(sReadStr);
		 ...
		 //: 日志地址
		 m_log_dir = ProfileAppString(Application.GetAppName(), "GENERAL", "LOG_DIR", "");
		 m_log_dir = AllTrim(m_log_dir);
		 m_log_dir = TrimCRLF(m_log_dir);
		 if (m_log_dir == "")
		 {
			 printf("请检查配置文件中的LOG_DIR字段\n");
			 exit(-1);
		 }
		 ForceDirectories(m_log_dir);
