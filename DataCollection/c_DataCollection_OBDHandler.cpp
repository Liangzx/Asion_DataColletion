...
void TCDataCollectionOBDHandler::DoCommand_ParseGpsInfo(TCCustomUniSocket & cusSocket)
{
	...
	for (size_t i = 0; i < gps_infos_.size(); i++)
	{
		...
		record.speed = (int)gps.speed_/10;
		record.direction_ = (int)gps_infos_[i].direction_;
		//
		...
		//
		std::cout << "DoCommand_ParseGpsInfo:" + record.putcsv() << std::endl;
		//
...
void TCDataCollectionOBDHandler::DoCommand_ParseLbsInfo(TCCustomUniSocket & cusSocket)
{
	...
	record.rscp_ = (int)AscStrToInt(Mid(msg_body, 20, 1));
	...
	record.unique_id_ = m_no_imei;
	//
	std::cout << "DoCommand_ParseLbsInfo:" + record.putcsv() << std::endl;
	//
	LocationRecordToRedis(record);
}
...
void TCDataCollectionOBDHandler::DoCommand_ParseLbsInfo_4G(TCCustomUniSocket & cusSocket)
{
	...
	record.vender = vender;
	//
	std::cout << "DoCommand_ParseLbsInfo_4G:" + record.putcsv() << std::endl;
	//
	LocationRecordToRedis(record);
}

void TCDataCollectionOBDHandler::DoCommand_ParseCar(TCCustomUniSocket & cusSocket)
{
	// 取gps信息
	TCString msg_body = Mid(m_sPkg_Content, 14, m_nMsgBodyLen);
	TCString gps_infos = Mid(msg_body, 12, 20);
	GpsInfo gps;
	gps.altitude_ = AscStrToInt(Mid(gps_infos, 10, 4));
	/*gps.bat_vol_ = AscStrToInt(Mid(gps_infos, (int)(gps_pkg_num * i + 26), 2));*/
	gps.direction_ = AscStrToInt(Mid(gps_infos, 16, 2));
	gps.latitude_ = AscStrToInt(Mid(gps_infos, 2, 4));
	gps.longitude_ = AscStrToInt(Mid(gps_infos, 6, 4));
	gps.speed_ = AscStrToInt(Mid(gps_infos, 14, 1));

	RecordInfo record;
	record.clear();

	record.elevation_ = (int)gps.altitude_;
	record.latitude_ = FloatToStr(gps.latitude_ / 1000000.0, 6);
	record.longitude_ = FloatToStr(gps.longitude_ / 1000000.0, 6);
	record.speed = (int)gps.speed_/10;
	record.direction_ = (int)gps.direction_;
	//
	record.time_stamp = TCTime::Now();
	record.imsi = m_no_imsi;
	record.unique_id_ = m_no_imei;
	record.vender = vender;
	//
	std::cout << "DoCommand_ParseCar:" + record.putcsv() << std::endl;
	//
	LocationRecordToRedis(record);
